...
- Before creating a Windows descriptor or lock, protect `%LOCALAPPDATA%\VibeMux\runtime` with a DACL granting full control only to the current user SID, `LOCAL_SYSTEM`, and built-in administrators. Hashed project leaves and their artifacts inherit only those effective rules.
...
- Verify the protected root and effective leaf/artifact ACLs. Broad principals such as Everyone, Authenticated Users, Users, or sandbox groups must not remain.
