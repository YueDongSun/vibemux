...
- `daemon start` launches `vibemuxd` with argv, null stdin/stdout/stderr, no shell, and no visible Windows console. POSIX uses a separate process group. The development-only daemon executable override is an explicit path argument and is never a command string.
