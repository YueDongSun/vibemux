...
- Local control IPC：`vibemuxd::control` 已实现 private pre-alpha v1 的 authenticated Windows named-pipe/POSIX UDS transport，仅开放 `health`/`shutdown`。descriptor含本地 bearer token并被忽略；没有TCP fallback。`vibemuxctl`只负责standalone daemon lifecycle与显式stale metadata recovery，不直接访问SQLite或暴露token/endpoint path。
