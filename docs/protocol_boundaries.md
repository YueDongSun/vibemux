...
- Local control IPC：`vibemuxd::control` 已实现 private pre-alpha v1 的 authenticated Windows named-pipe/POSIX UDS transport，仅开放 `health`/`shutdown`。descriptor含本地 bearer token并被忽略；没有TCP fallback，也尚无 standalone daemon/CLI lifecycle contract。
