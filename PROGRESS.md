...
- [x] Windows protected current-user control runtime and cross-user/remote access boundary (effective root/leaf/artifact ACLs re-verified in one batched helper pass on every trusted daemon start — issue #7 Stage 1, 2026-09-23).
...
> **Known deviation (2026-09-23, issue #7 Stage 1):** trusted daemon starts now re-verify the control-runtime ACLs in one batched PowerShell helper pass (ADR-020 line 18), adding ~400 ms per start on the benchmark machine (start p50 903.22 ms / p95 920.31 ms, 20 samples, release, existing database) — above the 500 ms p95 target, which exempts only the one-time bootstrap. Stage 2 (native read-only ACL verification, no process spawn) is the planned remedy; see the 2026-09-23 (3) log entry for the full numbers and baseline.
