...
**Tests**
- Platform: 5 native-module tests (fixed-SID wire layouts round-trip through `ConvertSidToStringSidW` to `S-1-5-18`/`S-1-5-32-544`; token SID readable; missing path → stage 4; unprotected tempdir rejected; concurrency/churn stress pin) + 1 new batched missing-index test (`stage 4, index 3`); all 6 Stage 1 windows_security tests green **unchanged** (they pin fail-closed behavior, not the implementation), including the `icacls`-loosened `AccessControlInvalidAt { stage: 7, index: 3 }` contract.
