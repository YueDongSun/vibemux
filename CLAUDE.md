...
All commands run from the repo root. PowerShell is the documented shell on Windows; the bash recipes below are equivalent.
...
If `inspect` reports `recoverable`, the output contains a 64-bit confirmation that must be passed to a separate `daemon recover` invocation. Recovery never kills a PID and never opens or modifies the database — it only removes unchanged stale descriptor/socket/writer lock artifacts.
...
- **Documentation ahead of implementation is forbidden.** If `PROGRESS.md` status moves, update it in the same PR. Commands in docs must match the current CLI.
