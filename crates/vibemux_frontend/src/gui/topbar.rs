...
pub fn render(
    ...
    // The chrome `TopBottomPanel` uses a top-down `Layout` (see egui
    // `containers::panel::TopBottomPanel::show_inside_dyn`), but the
    // topbar paints a horizontal row of controls. Wrap the contents
    // in an explicit `ui.horizontal(...)` block so the brand mark,
    // workspace label, and right-aligned toolbar all land on a single
    // line — without this the widgets stack vertically inside the
    // ~30 px default chrome height and most of them get clipped.
    ui.horizontal(|ui| {
